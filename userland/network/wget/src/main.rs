...
use libc_lite::{
    close, connect, gethostbyname, println, print, recv, send, socket, write,
...
fn split_url(s: &[u8]) -> (&[u8], u16, &[u8]) {
    ...
    let port = if let Some(c) = colon {
        let end = if path_start < s.len() { path_start } else { s.len() };
...
fn print_u16(n: u16) {
    ...
    let mut i = 0;
    let mut v = n;
    if v == 0 { let _ = write(1, b"0"); return; }
...
fn print_ip(ip: [u8; 4]) {
    print_u16(ip[0] as u16); print(".");
    print_u16(ip[1] as u16); print(".");
    print_u16(ip[2] as u16); print(".");
...
pub extern "C" fn main(argc: i32, argv: *const *const u8) -> i32 {
    ...
    let fd = match socket(AF_INET, SOCK_STREAM, 0) {
        Ok(f) => f,
        Err(_) => { println("wget: socket failed"); return 1; }
    };
    ...
    let mut tmp = [0u8; 20];
    let mut i = 0;
    if total == 0 { tmp[0] = b'0'; i = 1; }
    let mut t = total;
    while t > 0 { tmp[i] = b'0' + (t % 10) as u8; t /= 10; i += 1; }
    let mut rev = [0u8; 20];
    for j in 0..i { rev[j] = tmp[i - 1 - j]; }
    let _ = write(1, &rev[..i]);
