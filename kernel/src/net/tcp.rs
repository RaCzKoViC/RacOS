...
impl TcpHeader {
    ...
    pub fn parse(data: &[u8]) -> Option<(Self, &[u8] /* options */, &[u8] /* payload */)> {
        ...
        if header_bytes < TCP_HDR_LEN || header_bytes > data.len() {
            return None;
        }
...
impl Table {
    ...
    fn find_mut(&mut self, local_port: u16, remote_ip: [u8; 4], remote_port: u16) -> Option<usize> {
        self.conns.iter().position(|c| {
            c.as_ref()
                .map_or(false, |x| x.matches(local_port, remote_ip, remote_port))
        })
    }
    ...
    fn alloc_port(&mut self) -> u16 {
        for _ in 0..1024 {
            ...
            self.next_ephemeral = if self.next_ephemeral >= 65000 {
                49152
            } else {
            ...
            let in_use = self
                .conns
                .iter()
                .any(|c| c.as_ref().map_or(false, |x| x.local_port == p));
            if !in_use {
                return p;
            }
        }
...
pub fn close(id: ConnId) -> Result<(), TcpError> {
    let (our_ip, our_mac, peer_mac, remote_ip, remote_port, local_port, seq, ack, win) = {
        ...
        conn.state = if conn.state == State::Established {
            State::FinWait1
        } else {
            State::LastAck
        };
...
pub fn handle_segment(ip: &ipv4::Ipv4Header, payload: &[u8]) {
    let Some((hdr, options, data)) = TcpHeader::parse(payload) else {
        return;
    };
...
fn process_segment(
    ...
    match conn.state {
        ...
        State::TimeWait | State::Closed => SegmentResult {
            notify: Notify::None,
            ack: None,
        },
    }
...
pub fn tick() {
    ...
    for (idx, slot) in t.conns.iter_mut().enumerate() {
        ...
        for seg in conn.pending.iter_mut() {
            ...
            eth::write_header(&mut buf, &conn.peer_mac, &our_mac, eth::ETHERTYPE_IPV4);
            let tcp_off = eth::ETH_HDR_LEN + ipv4::IPV4_HDR_LEN;
            let mss_opt = if seg.flags & SYN != 0 {
                Some(DEFAULT_MSS)
            } else {
                None
            };
